...
export function parseYaml(input: Uint8Array | string, options: YamlParseOptions = {}): ParseOutcome<YamlDocument> {
 ...
 try { const ast = options.deps.parse(text); const maxDocs = options.limits?.maxDocuments ?? 100; const docs: YamlNode[] = [];
...
function validateTree(root: YamlNode, maxNodes: number, maxDepth: number, maxAliases: number): string | null { let nodes = 0, aliases = 0; const stack: Array<{ node: YamlNode; depth: number }> = [{ node: root, depth: 0 }]; while (stack.length) { const item = stack.pop()!; nodes++; if (nodes > maxNodes) return 'yaml.node-limit'; if (item.depth > maxDepth) return 'yaml.depth-limit'; if (item.node.kind === 'alias' && ++aliases > maxAliases) return 'yaml.alias-limit'; for (const child of item.node.children ?? []) stack.push({ node: child, depth: item.depth + 1 }); } return null; }
