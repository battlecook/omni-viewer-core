...
import type { ArchiveDecoder, ArchiveDocument, ArchiveOpenOptions, ArchiveStreamDecoder, OpenArchiveHandle } from './model.js';
...
export interface ArchiveParseOptions extends ParseOptions { fileName?: string; password?: string; }
export interface ArchiveStreamOptions { fileName?: string; totalSize?: number; signal?: AbortSignal; limits?: ResourceLimits; password?: string; }
export async function parseArchive(data: Uint8Array, decoder: ArchiveDecoder, options: ArchiveParseOptions = {}): Promise<{ outcome: ParseOutcome<ArchiveDocument>; handle?: OpenArchiveHandle }> {
    ...
    try { const raw=await decoder.openArchive(data,openOptionsFrom(options.limits,options.signal,options.password)); return finalizeOpen(raw, options.limits, started); } catch (error) { return openError(error, started); }
}
...
export async function openArchiveStream(source: ArchiveStreamDecoder, options: ArchiveStreamOptions = {}): Promise<{ outcome: ParseOutcome<ArchiveDocument>; handle?: OpenArchiveHandle }> {
    ...
    if (options.signal?.aborted) return failed('aborted','diag.aborted',started,true,true);
    try { const raw=await source.openArchive(openOptionsFrom(options.limits,options.signal,options.password)); return finalizeOpen(raw, options.limits, started); } catch (error) { return openError(error, started); }
}
function openOptionsFrom(limits: ResourceLimits | undefined, signal: AbortSignal | undefined, password: string | undefined): ArchiveOpenOptions { return { maxEntries: limits?.maxEntries ?? ARCHIVE_DEFAULT_LIMITS.maxEntries, maxDecompressedBytes: limits?.maxDecompressedBytes ?? ARCHIVE_DEFAULT_LIMITS.maxDecompressedBytes, ...(signal ? { signal } : {}), ...(password !== undefined ? { password } : {}) }; }
function finalizeOpen(raw: OpenArchiveHandle, limits: ResourceLimits | undefined, started: number): { outcome: ParseOutcome<ArchiveDocument>; handle: OpenArchiveHandle } {
    ...
    const document: ArchiveDocument = { entries, ...(raw.encrypted !== undefined ? { encrypted: raw.encrypted } : {}) };
    return { handle, outcome:{result: partial ? {status:'partial',document,diagnostics:[{severity:'warning',code:'archive.limit-exceeded',messageKey:'diag.archive.limit-exceeded'}]} : {status:'ok',document,diagnostics:[]},execution:{workerUsed:false,hardLimitEnforced:false,elapsedMillis:Date.now()-started}} };
}
...
function limitHandle(raw: OpenArchiveHandle, maxTotal: number): OpenArchiveHandle { let used = 0; let reserved = 0; return { entries: raw.entries, ...(raw.encrypted !== undefined ? { encrypted: raw.encrypted } : {}), async extract(id, opts) { const allowance=Math.min(opts.maxBytes,maxTotal-used-reserved); if (allowance <= 0) throw new ArchiveError('limit-exceeded','diag.archive.limit-exceeded'); reserved += allowance; try { const bytes=await raw.extract(id,{...opts,maxBytes:allowance}); used += bytes.byteLength; if (bytes.byteLength > allowance || used > maxTotal) throw new ArchiveError('limit-exceeded','diag.archive.limit-exceeded'); return bytes; } finally { reserved -= allowance; } }, close: () => raw.close() }; }
