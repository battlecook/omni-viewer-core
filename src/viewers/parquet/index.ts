...
export async function mountParquetViewer(input:ViewerInput|ParquetViewerSource,container:HTMLElement,ctx:ParquetViewerContext,options:ParquetMountOptions={}):Promise<ViewerHandle>{
 ...
 const ctl=createParquetController(doc),off:Array<()=>void>=[];let disposed=false,loadingMore=false;let root:HTMLElement|ShadowRoot=container;
 ...
 search.type='search';search.placeholder=ctx.i18n.t('parquet.search');bar.append(search,view,copyTableButton,copyJsonButton,save,info);
 ...
 if(ctx.clipboard){on(copyTableButton,'click',()=>void copyTable());on(copyJsonButton,'click',()=>void ctx.clipboard!.writeText(ctl.toJson()))}else{for(const button of [copyTableButton,copyJsonButton]){button.disabled=true;button.title=ctx.i18n.t('common.noClipboard')}}
 ...
 async function copyText(text:string){if(ctx.clipboard)await ctx.clipboard.writeText(text)}
 ...
 async function loadNext(){if(loadingMore||disposed||!doc.isLimited)return;loadingMore=true;loadError.textContent='';renderStatus();try{const chunk=await runParse({...parseOptions,rowStart:doc.loadedRows});if(!disposed)ctl.appendRows(chunk.rows)}catch(error){if(!disposed&&!options.signal?.aborted)loadError.textContent=ctx.i18n.t('parquet.loadFailed',{message:error instanceof Error?error.message:String(error)})}finally{loadingMore=false;if(!disposed)renderStatus()}}
