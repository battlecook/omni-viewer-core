...
/** Highlights pretty-printed JSON (always well-formed `JSON.stringify` output),
 * colouring object keys apart from string values, numbers and literals. */
function highlightJson(target: HTMLElement, text: string): void {
    target.replaceChildren();
    const span = (className: string, value: string): HTMLSpanElement => { const node = document.createElement('span'); node.className = className; node.textContent = value; return node; };
    ...
    while ((match = token.exec(text))) {
        ...
        const value = match[0];
        if (value[0] === '"') target.appendChild(span(/^\s*:/.test(text.slice(token.lastIndex)) ? 'omni-structured__json-key' : 'omni-structured__json-string', value));
        else if (value === 'true' || value === 'false') target.appendChild(span('omni-structured__json-boolean', value));
        else if (value === 'null') target.appendChild(span('omni-structured__json-null', value));
        else target.appendChild(span('omni-structured__json-number', value));
